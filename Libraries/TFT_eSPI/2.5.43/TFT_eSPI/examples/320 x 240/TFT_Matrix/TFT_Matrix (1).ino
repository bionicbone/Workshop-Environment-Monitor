...
#define SCREEN_VIRTICAL_PIXELS 480



uint16_t yStart = TOP_FIXED_AREA;
uint16_t yArea = SCREEN_VIRTICAL_PIXELS - TOP_FIXED_AREA - BOT_FIXED_AREA;
uint16_t yDraw = SCREEN_VIRTICAL_PIXELS - BOT_FIXED_AREA - TEXT_HEIGHT;
byte     pos[135];
uint16_t xPos = 0;
...
void loop(void) {
  ...
  for (int j = 0; j < SCREEN_VIRTICAL_PIXELS + 20; j += TEXT_HEIGHT) {
    for (int i = 0; i < 133; i++) {
      if (pos[i] > 20) pos[i] -= 3; // Rapid fade initially brightness values
      if (pos[i] > 0) pos[i] -= 1; // Slow fade later
      if ((random(20) == 1) && (j < SCREEN_VIRTICAL_PIXELS +20)) pos[i] = 63; // ~1 in 20 probability of a new character
  ...
  while (1) { yield(); yDraw = scroll_slow(SCREEN_VIRTICAL_PIXELS, 5); }// Scroll 320 lines, 5ms per line
...
void setupScrollArea(uint16_t TFA, uint16_t BFA) {
  ...
  tft.writedata(TFA >> 8);
  tft.writedata(TFA);
  tft.writedata((SCREEN_VIRTICAL_PIXELS - TFA - BFA) >> 8);
  tft.writedata(SCREEN_VIRTICAL_PIXELS - TFA - BFA);
  tft.writedata(BFA >> 8);
  tft.writedata(BFA);
}
...
int scroll_slow(int lines, int wait) {
  ...
  for (int i = 0; i < lines; i++) {
    yStart++;
    if (yStart == SCREEN_VIRTICAL_PIXELS - BOT_FIXED_AREA) yStart = TOP_FIXED_AREA;
